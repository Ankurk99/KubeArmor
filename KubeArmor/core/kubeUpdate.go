...
func (dm *KubeArmorDaemon) ParseAndUpdateContainerSecurityPolicy(event tp.K8sKubeArmorPolicyEvent) {
	...
	if event.Type == "ADDED" {
		...
		newPoint.PolicyEnabled = tp.KubeArmorPolicyEnabled
		newPoint.Identities = []string{"namespaceName=" + secPolicy.Metadata["namespaceName"]}
		newPoint.ProcessVisibilityEnabled = true
		newPoint.FileVisibilityEnabled = true
		newPoint.NetworkVisibilityEnabled = true
		newPoint.CapabilitiesVisibilityEnabled = true
...
// WatchContainerSecurityPolicies Function
func (dm *KubeArmorDaemon) WatchContainerSecurityPolicies() {
	for {
		if !K8s.CheckCustomResourceDefinition("kubearmorcontainerpolicies") { //TODO
			time.Sleep(time.Second * 1)
			continue
		}

		if resp := K8s.WatchK8sHostSecurityPolicies(); resp != nil { //TODO
			defer resp.Body.Close()
			...
			for {
				...
				dm.ParseAndUpdateContainerSecurityPolicy(event)
			}
		}
	}
}
